...
public class ReqLimitIntercepter implements HandlerInterceptor {
    ...
    public void isLimit(String ipAddress , String name ,RequestLimit reqLimit) throws BlockException {
        ...
        if (redisTemplate.hasKey(blockKey)) {
            ...
            throw new BlockException(Code.REQUEST_BLOCK, Message.REQUEST_BLOCK);
        } else {
            // 到redis查该ip的访问次数
            ...
            if (cnt != null) {
                ...
                if (cnt >= reqLimit.count()) {
                    // 被block
                    log.info("ip: " + ipAddress + " 请求方法: " + name + " 被block了");
                    redisTemplate.opsForValue().set(blockKey, 1, reqLimit.blockTime(), java.util.concurrent.TimeUnit.MINUTES);
